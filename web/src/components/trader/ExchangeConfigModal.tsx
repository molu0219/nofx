...
export function ExchangeConfigModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (
        ...
        // Paper trading needs no API keys — the trader's initial_balance is the
        // virtual wallet balance. Empty credentials are written; backend handles
        // defaults (5 bps fee, $10k starting balance unless overridden by trader).
        await onSave(
          exchangeId,
          exchangeType,
          trimmedAccountName,
  ...
  const isPaper = currentExchangeType === 'paper'
        ...
        >
          ...
          {currentStep === 0 && !editingExchangeId && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Paper trading — virtual venue, no credentials needed; available
                    even when the WebCrypto check would otherwise gate real exchanges. */}
          ...
          {(currentStep === 1 || editingExchangeId) && selectedTemplate && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* Paper-trading explainer — replaces the API-key flow entirely. */}
              {isPaper && (
                <div
                ...
                >
                  ...
                  <ul className="text-xs space-y-1 list-disc pl-5">
                    <li>
                      No API keys, no real funds, no exchange to sign up for.
                    </li>
                    <li>
                      Live mark prices come from the same market data feed real
                      traders use.
                    </li>
                    <li>
                      Margin lock, isolated/cross liquidation, funding accrual,
                      stop-loss/take-profit triggers all behave like a real perp
                      venue.
                    </li>
                    <li>
                      Set the starting wallet balance later when you create a
                      trader on this exchange — that becomes the virtual
                      capital.
                    </li>
                    <li>State persists across restarts via the local store.</li>
                  </ul>
                </div>
              )}
