...
export function TraderDashboardPage({
  ...
  const isPaper = currentExchange?.exchange_type === 'paper'

  // Reset paper account: confirm with the user, call the per-trader reset
  // endpoint, then refresh the dashboard data so the cleared positions /
  // restored balance show immediately.
  const handleResetPaper = async () => {
    if (!selectedTrader) return
    const confirmed = await confirmToast(
      t('traderDashboard.resetPaperConfirmMessage', language),
      {
        title: t('traderDashboard.resetPaperConfirmTitle', language),
        okText: t('traderDashboard.resetPaperConfirm', language),
    ...
    try {
      ...
      await Promise.all([
        mutate(`/api/account?trader_id=${selectedTrader.trader_id}`),
        mutate(`/api/positions?trader_id=${selectedTrader.trader_id}`),
        mutate(
          `/api/decisions/latest?trader_id=${selectedTrader.trader_id}&limit=5`
        ),
      ])
    } catch (err) {
      const message = err instanceof Error ? err.message : 'reset failed'
      notify.error(
        `${t('traderDashboard.resetPaperFailure', language)}: ${message}`
      )
    }
  }
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Reset paper account: visible only on paper exchanges. */}
              {isPaper && (
                <button
                  type="button"
                  onClick={handleResetPaper}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg nofx-glass border border-nofx-gold/20 hover:border-nofx-gold/50 transition-colors"
                  title={t('traderDashboard.resetPaperTooltip', language)}
                >
                  <RotateCcw className="w-3.5 h-3.5 text-nofx-gold" />
                  <span className="text-xs text-nofx-gold">
                    {t('traderDashboard.resetPaper', language)}
                  </span>
                </button>
              )}
